...
If you are an AI agent (Codex, codex, any other) about to touch this
repository, this file is your boot sequence. Read it before you run any
command, open any branch, or write any file.

> **Filename note:** named `AGENTS.md` because Codex auto-loads files
> with that name from anywhere in the repo, so every new Codex
...
> agent on the team, not just Codex.
...
1. **[`agent-system/PARALLEL_WORK.md`](agent-system/PARALLEL_WORK.md)** —
   ...
   rule, the **master-merger policy** (only `Codex-main` runs `gh pr merge`
...
2. **[`docs/PROJECT_BACKLOG.md`](docs/PROJECT_BACKLOG.md)** — single source
   ...
   same PR.
3. **[`apps/api/AGENTS.md`](apps/api/AGENTS.md)** — top backend traps
...
- **Don't merge to `main` yourself.** Open a PR; `Codex-main` reviews and
  merges. If `Codex-main` is offline and a merge is urgent, escalate to
...
- **Always prefix branches with your agent name.** `Codex/feat/<slug>`,
...
- **Work in your own worktree, not the canonical root.** Canonical root
  (`/Users/jimmygu/the_counselor`) is `Codex-main`'s home and stays on
...
- **Karpathy coding principles** apply by default: think before coding,
  ...
  Persisted in the canonical Codex user-memory as
...
- **Python 3.9 compat** for backend — use `Optional[X]`, not `X | None`,
  ...
  is documented in `apps/api/AGENTS.md`.
...
You auto-load this `AGENTS.md` plus any other `AGENTS.md` files in the repo
(see `apps/api/AGENTS.md`). On Jimmy's primary machine you also auto-load
user memory at `~/.Codex/projects/-Users-jimmygu/memory/` which has
...
## For non-Codex agents (codex, other tools)
...
*Created 2026-05-21 by `Codex-main` (master merger).*

## Imported Claude Cowork project instructions

Leverage market research, product manager and UX expert skills when completing this task, always provide clear and concise answers with data backed hypothesis
