...
### Episode 35 — Reshaping PRD-19 Phase B: the reverted feature that needed three latent bugs caught before it could ship again (June 8 → 9)

**Backdrop.** PRD-19 Phase B (signal-change emails + daily digest) first shipped as PR #88 back on 2026-05-26, then got swept up in the 16-hour Railway outage that day and unnecessarily reverted via PR #101 — the Railway-deployment-ID-as-git-SHA confusion captured in Episode 27. Six weeks later, Jimmy decided to re-attempt under a tighter PRD spec (the "PRD-19 reshape") with a clear sequence: model + endpoint first (3a), cron dispatch wiring second (3b), then user-facing controls (4a → 4b → 4c), then frontend (5 → 6).

The session opened with a clean-up. Sonnet 4.6's prior session had landed [`apps/api/app/main.py`](apps/api/app/main.py) referencing `notifications.py` but forgotten to `git add` the module. CI was red for 38 minutes; production stayed up only because Railway held the prior container. PR #146 fixed the missing import. PR #147 codified the "static-import smoke test" as pre-push checklist item #6 (`python3 -c "from app.main import app; print(len(app.routes))"`) so the next forgotten-add never makes it past commit. The cost of that 2-second check is now amortized over every PRD-19 PR — caught nothing new this session, but the bug class is preventable forever.

Then the master-merger handshake. Jimmy proposed (and we shipped in PR #147) a verbal convention: whoever is the master merger this session addresses Jimmy as **"Mr Gu"** in their first reply each turn. Non-master sessions use "Jimmy" or no greeting. It makes the active role visible at a glance — and if a session greets Jimmy as "Mr Gu" but isn't actually master merger, Jimmy can correct it in one line.

Then PRD-19, in five sequential single-PR slices, all under the `claude/` prefix with claude-main as master merger.

**PR #150 — Step 3a: Mark-as-Executed.** New SQLAlchemy model + migration + the `POST /api/saved-strategies/{id}/mark-executed` endpoint that closes the retention loop's user side. UNIQUE index on `(user_id, signal_event_id)` backs idempotency at the DB layer; the endpoint handles repeat-clicks gracefully. PostHog `notification_executed` captured on first click only — duplicate clicks do NOT re-fire, preserving the metric's integrity. 12 tests covering happy path, idempotency, latest-not-earliest signal selection, 404 cases, clock-skew clamping. Pushed CI green. **Found a real bug post-CI:** the route imported `ph_capture` from `posthog_service`, but the module only exports `capture`. The `try/except` wrapper would have silently swallowed the `ImportError` in production — the test passed because monkeypatch was creating the attribute. Fix-up commit replaced the import. Lesson logged: when patching in a test, prefer patching the real exported name over `raising=False` which silently lets typos through.

**PR #152 — Step 3b: dispatcher wiring (re-push of #151).** The big one. New `app/emails/signal_change.py` with `render_signal_change(user, payload)` — modeled on `welcome.py`, real html+text pair, `make_unsub_token(user.id, f"signal_alerts_{strategy_id}")` signed unsub URL, CAN-SPAM footer + "Not investment advice" boilerplate. `dispatch_signal_change_email(event, db, user)` refactored to use the new renderer + correct `send_email(db, user, *, template, subject, html, text, category)` signature — the existing dispatcher had been calling `send_email(to=..., subject=..., body_html=...)`, a signature that never existed; every cron tick would have logged `"Failed to send"` once and delivered zero emails. The legacy inline `_render_signal_change_email` (with literal `{{unsubscribe_url}}` Jinja-style tokens that nothing would have substituted) deleted.

`signal_cron` looks up `User` once per subscribed flip, passes to the dispatcher; on send-success, sets `SignalEvent.email_dispatched_at = utcnow()` and `email_dispatch_count += 1`. PostHog `notification_dispatched` captured on every attempted send with `email_sent` + `in_app_banner_sent` + the joinable `signal_event_id`. PostHog `notification_throttled` captured with `reason: strategy_daily_cap | user_daily_cap` so suppressed dispatches are visible — silent throttling is invisible throttling.

The integration test caught the third latent bug pre-merge: the in-memory throttle counters (`strategy_daily_counts`, `user_daily_counts`) reset to 0 on every cron tick because they were local variables. APScheduler restart or misfire-recovery → second email-per-strategy-per-day, violating the documented daily cap. The fix: `_seed_throttle_counters` pre-fills the dicts from any `SignalEvent.email_dispatched_at` rows landing in today's UTC window — throttle now survives cron restarts.

PR #151 opened, CI green, then Jimmy closed it and head-ref-deleted at 11:20 UTC. I asked rather than re-pushing silently — turned out to be no specific objection, "re-push as a fresh branch" was the answer. PR #152 went up under `claude/feat/signal-cron-dispatch-2`, same commit, same scope, CI green again, self-merged after Mr Gu's "go for it." Lesson: when the merge is unexpected, ask. The cost of asking is a single message; the cost of replaying work that was deliberately rejected is a multi-PR round trip.

**PR #153 — Step 4a: preference flags + trap-#16 drive-by.** Three new boolean fields on `EmailPreference` (`signal_alerts_enabled`, `daily_digest_enabled`, `silent_days_enabled`), each in its own `engine.begin()` mini-tx per trap #3 with SQLite try/except + Postgres `IF NOT EXISTS` per trap #6. Extended `GET/PATCH /api/me/email-preferences` (the existing endpoint — single surface, not parallel) to read + write them. Extended `_prefs_allow(prefs, template, category)` so the new flags win over the transactional default for templates `signal_change` and `daily_digest`. Legally-required transactional templates (`password_reset`, `payment_failed`) still bypass — CAN-SPAM table stakes.

The drive-by: the Step 3b throttle test was passing in CI containers (which run in UTC) but failed the moment Step 4a's worktree opened in local TZ. `signal_cron.py` was computing `today = date.today()` (local) but writing `email_dispatched_at = datetime.utcnow()` (UTC). The seeding query missed its own events at the TZ boundary. Fix: `today = datetime.utcnow().date()`. That's trap #16 catching its own regression — the cross-tick throttle test only worked because it exercised the seed path, and the seed path only broke when the test ran in a non-UTC TZ. CI containers had been hiding it.

**PR #154 — Step 4b: daily digest.** New `app/emails/daily_digest.py` with `render_daily_digest(user, payload)`, modeled on signal_change.py — color-coded rows (amber=changed, green=stable, slate=cash), signed `daily_digest` unsub URL. New `app/jobs/daily_digest_job.py` enumerates users with active SignalAlertSubscriptions, gates on `daily_digest_enabled` + `unsubscribed_at`, buckets each subscribed strategy into changed/stable/cash, honors `silent_days_enabled` via `notification_throttle.should_skip_digest`. PostHog `daily_digest_dispatched` / `daily_digest_skipped_silent_day` events. `DigestEvent` schema extended with `cash_count: int = 0` (the bucketing test caught the missing field — the headline counter only included `changed_count` and `stable_count`). Cron registered at 13:00 UTC (~9am ET) — after the previous trading day's 22:00 UTC signal_cron tick has written all the SignalEvents. The same trap-#21 pattern (sync wrapper around the work) but no `asyncio.run` needed because the body is pure sync DB.

**PR #155 — Step 4c: unsub webhook extension.** Last backend slice. The signal-change email's signed `signal_alerts_<strategy_id>` token and the digest email's `daily_digest` token were both being minted correctly upstream but falling through to "expired or invalid" in the switch ladder at `/api/email/unsub`. Added `category == "daily_digest"` → flips `prefs.daily_digest_enabled = False`. Added `category.startswith("signal_alerts_")` → parses strategy_id suffix, flips `SignalAlertSubscription.email_enabled = False` for `(user_id, strategy_id)`. The `category == "all"` arm extended to also flip the PRD-19 flags — otherwise "Unsubscribe from all marketing" would leave signal alerts (`transactional`) and digests on.

Anti-enumeration preserved throughout: every code path returns HTTP 200 with the same friendly HTML page. The tampered-HMAC test pins it explicitly. The composite-PK lookup (`db.get(SignalAlertSubscription, (user_id, strategy_id))`) no-ops silently if the row's gone — token may post-date subscription deletion.

**Backend test count: 855 → 884 (+29 across 5 PRs). Zero regressions across the sequence.** Three latent bugs from the original PR #88 caught pre-merge: wrong `send_email` signature in dispatcher, literal `{{unsubscribe_url}}` tokens in compliance footer, in-memory throttle counters resetting across cron ticks. Plus the trap #16 fix that surfaced because the worktree TZ ≠ container TZ. Plus the ph_capture vs capture import slip caught in the fix-up commit on Step 3a.

**The cross-cutting lesson.** Every feature reshape that defines new categories / templates / token shapes has to grow every GATE / SWITCH / RENDER that handled the old shapes. If the new branch is missing, the new shapes silently fall through to a no-op or a default — and "no-op" looks identical to "shipped" until a user clicks. The three latent bugs in PRD-19 weren't independent: they all came from the same reshape-without-following-through pattern. Codified in `docs/LEARNINGS.md` as "Template literals that look like substituted strings — grep them BEFORE shipping."

**The session shape.** Single-PR slices, single master merger, single worktree per slice off main, Monitor-watched CI, "Mr Gu" handshake every turn, "ask before merging when unexpected things happen" applied at the #151 close. Zero rollbacks. Zero stacked PRs (every PR was `base=main` so backend CI fired on every one). Three follow-up Steps 5+6 to land later — backend is stable, frontend is separable.

**PRD-19 backend complete with #155.**
...
### Episode 39 — PRD-16c, the whole thing, plus the reachability audit that almost shipped it broken (June 9, late)

Same continuous session as Episodes 35–38 — somewhere around hour 12. PRD-19 + 16a + 16b were on main; PRD-16c was the largest of the three (intraday data layer, multi-tier exits, position state, monitor cron, live dashboard, composer extension) and the spec ran 762 lines. Mr Gu's directive was three words: "let's keep moving finishing up PRD16 entirely."

Eight backend + frontend slices, each base=main, each auto-merged after green CI:

**16c-1 (#171) — the data foundation.** `IntradayBarService` fronting an `intraday_bars` SQLite/Postgres cache (composite PK `(symbol, resolution, bar_time)`) with delete-then-insert upsert. Graceful AV-failure handling: returns stale cached bars rather than empty when the upstream errors. AV's intraday endpoint has one quirk worth pinning — the time-series key is hyphenated (`Time Series (15min)`) not underscored (the daily endpoint uses `Time Series (Daily)`). Test caught a typo in the first commit. 12 tests.

**16c-2 (#172) — the engine and the schema.** `BacktestEngine.run` gained a `bar_resolution: str = "daily"` parameter with default preserving every existing 22 strategy_types' behavior. Non-daily currently raises `NotImplementedError` pointing at 16c-3 (the intraday data path is wired but the engine's bar-series consumption needs the cron + position state to be useful). `ExitTier` and `RiskManagement.exit_ladder` schemas with full validators (≥1 stop tier, ascending order, fraction in (0,1)). The load-bearing piece: `_apply_exit_ladder`, a position-aware weight-matrix post-processor that runs between `_generate_weights` and the returns computation. Tracks entry price per symbol, fires each tier AT MOST ONCE per entry, scales weight cumulatively on `sell_fraction`, zeros forward on `sell_all`, resets state when the strategy itself closes a position. 22 tests including the canonical SpaceX 3-tier ladder firing in order on a synthetic price path.

**The pitfall C re-run.** The same load-bearing guarantee from PRD-16b applied here — "additive change must not regress the 22 existing strategy_types." Verified by re-running the full backend suite after every slice that touched the engine or schema (1334 → 1346 → 1368 → 1376 → 1389 → 1403 → 1427 → 1431, all green). No diff inspection. Full suite, every time.

**16c-3a/b/c (#173/#174/#175) — the state machine.** Split the original 16c-3 into three because the original scope was too large for one PR landing surface:
- **3a**: `PositionState` ORM with composite-PK-safe FK to `saved_strategies.id` (both `String(36)` — no FK-mismatch trap), compound `idx_position_open_per_strategy` for the cron's hot path query.
- **3b**: `monitor_active_positions` cron with sync APScheduler wrapper around async core (trap #21). Deliberately uses `IntradayBarService` and NOT `live_quote_service` — the live quote singleton holds `asyncio.Lock` instances bound to the main loop, and the cron runs on a worker thread loop. Using the singleton would either race the main loop to bind the lock and then RuntimeError on user requests, or wedge the locks after an early-return exception. Trap #22 codified the rule after a Market Pulse outage; PRD-16c was the first feature work to apply it. Per-position throttle key adds symbol + trigger_type to the existing `(strategy_id, date)` keying so a stop on AAPL doesn't batch with a TP1 on AAPL doesn't batch with a stop on NVDA.
- **3c**: 3 owner-only dashboard endpoints on `/api/saved-strategies/{id}/`. The `universe-state` endpoint reads from the intraday cache only, never an AV roundtrip on the GET path — the monitor cron is responsible for freshness, the user-facing read is cache-only. Cold cache reports `source='no_data'` truthfully.

**16c-4 (#176) — the editorial + presentation slice.** Single-renderer `render_position_event` for all `stop_hit / tp1_hit / tp2_hit` types via a `_TRIGGER_META` lookup table. Same visual style across the three, no copy drift, fallback to neutral copy for unknown triggers. The catalog's intraday metadata extension is the small editorial choice with the largest downstream impact: `_INTRADAY_ELIGIBLE_IDS` is a deliberate whitelist of ~35 ids, not a "if data_source=price" auto-classifier. KAMA + SAR are mechanically eligible but tuned for daily and stay daily-only. Fundamentals + sentiment + cross-sectional ranking stay daily-only by intent. Tests pin both the whitelist and the "daily always first in resolution list" invariant that keeps PRD-16a-1's ETag cache stable on no-op deploys.

**16c-5 (#177) — flipping the disabled flag.** PRD-16b had shipped `<CustomBuildActiveExecutionScaffold>` as a visible-but-disabled placeholder (pitfall B: "leave room for 16c"). 16c flipped `disabled={false}` and revealed `<BarResolutionPicker>` (5-radio pill row: daily / 60min / 30min / 15min / 5min) and `<ExitLadderEditor>` (per-tier label + trigger % + action + fraction) inside an emerald-tinted card when the toggle's on. The editor's inline `validateExitLadder` mirrors the backend validator — surfaces "Need at least one stop tier" / "Tiers must be ordered ascending" / "Partial-out fraction must be between 0 and 1" in a rose-tinted box so the user never sees a backend 422. "Use SpaceX defaults" button populates the canonical `Stop -10 / TP1 +15 1/3 / TP2 +30` ladder. 19 tests.

**16c-6 (#178) — the live dashboard.** Three polling bricks: `<UniverseWatchPanel>` (cards per ticker with latest cached intraday price, 30s poll, skeleton + no-data + error states), `<PositionCardsGrid>` (open + closed cards with entry vs current, color-coded signed pct, mini trade-log timeline), `<TradeLogTable>` (paginated chronological event log via `?before=` cursor). All three respect trap #19 — fire only after `useSession()` resolves. `<ActiveExecutionDashboard>` is the composition wrapper.

After 8 slices, the work was technically complete. I almost stopped there. Then I asked Mr Gu the wrap-up question:

> "How does the user enter Custom Mode?"

The answer was: they don't. Two reachability gaps that the per-slice tests couldn't catch:

1. **`<EntryModePicker>` on the Home page had three CTAs — Pick asset / Upload portfolio / Chat builder.** Custom Build wasn't one of them. The flow definition's `triggers` array referenced `"strategy_builders/custom_build_cta"` but `grep -rn "startFlow.*custom_build_mode"` returned zero matches. The only way to reach the composer was typing `/flow/custom_build_mode` into the URL bar. And even users who got there hit a `compose_signals` step with `next: () => null` — the flow was terminal, logged `"completed (v1 no-op)"`, and dropped them on the floor.

2. **`<ActiveExecutionDashboard>` existed as a brick + tests; nothing imported it.** Strategy detail page didn't render it. The slug-keyed route returned a BacktestRecord; the dashboard endpoints were UUID-keyed on `SavedStrategy.id`. No bridge.

PR #179 closed (1) — replaced the Chat-builder tile with **"Build from scratch"** (Wand2 icon, primary tile), wired `startFlow("custom_build_mode", …)`, and along the way replaced the dead-end `compose_signals → null` step with a proper chain (`compose_signals → backtest → review → save`) reusing the mode-agnostic FlowBacktest / FlowReview / FlowSave bricks that `one_asset_mode` already uses. The canvas got an explicit "Run backtest →" CTA that synthesizes the StrategyJson via `buildCustomBuildStrategyJson` and advances. Disabled when rules empty / symbol blank / exit_ladder invalid; error surfaces inline.

PR #180 closed (2) with a single optional field: `SavedStrategyResponse.saved_strategy_id: Optional[str] = None`. Backend looks up `SavedStrategy.id WHERE backtest_record_id = record.id`. Frontend conditionally renders the dashboard: `data.saved_strategy_id && bar_resolution !== "daily" && <ActiveExecutionDashboard strategyId={...} />`. Non-owners hitting the dashboard's polls get 404 → the brick's built-in error state. No leakage. No new page.

**The pattern that came out of this:** *Per-slice tests verify the brick renders; end-to-end audit verifies a user can actually reach it.* When the work crosses many bricks, the integration layer between them is its own surface area. Same lesson as PRD-19 Step 5/6 (the banner deep-links to the settings page). Almost shipped both PRD-16c and PRD-19 with the brick existing and unreachable. The wrap-up question — "how does the user enter this?" — is the discipline that caught it both times.

**Cumulative session totals across PRD-19 + PRD-16a + PRD-16b + PRD-16c + UX wire-up: 34 PRs, +576 backend tests (855 → 1431), +107 frontend tests (75 → 182), 0 outages, 0 regressions, 10+ latent bugs caught pre-merge.** The Custom Mode 3-PRD packet (16a + 16b + 16c) is complete and end-to-end wired. From a user perspective: click a tile on Home, compose a strategy with active execution, click Run backtest, save, land on a strategy detail page with a live dashboard polling every 30s. From an engineering perspective: 8 traps (#16 through #22) respected throughout, backwards-compat verified by full-suite re-run after every additive change, the slug ↔ UUID bridge proves that the right small backend field can save a whole frontend page.
...
### Episode 46 — The empty tab, and building a machine that reasons about supply chains (July 21–24)

The Supply Chain tab was live but empty. Every company showed "insufficient evidence." The plumbing — edges, evidence tiers, a chokepoint verdict, a 4th analytical lens — had all shipped. It just had nothing to say. This is the story of filling it, and then of building the thing it was always meant to become: a machine that reasons about a company's supply-chain position and grades every claim by the evidence behind it.

**Peeling the onion.** Turning extraction on produced `{"ok": false}`. The temptation was to theorize from the config — and I did, wrongly, blaming a DeepSeek API key that had been pasted into a model-name slot. The Railway *traceback* told the real story: `TypeError: 'bool' object is not callable`. `has_content` was a `@property`; the code called it `has_content()`. One character. Fix it, and the next layer appears: `ok:true` now, but `edges:0` — in 4.5 seconds, far too fast for an LLM to have read a 10-K. The parser was matching "Item 1. Business" in the **table of contents** and handing the model a page-number listing. Fix *that* — scan every header, keep the longest body — and the model finally sees the real filing and extracts a real, quote-verified relationship. Three bugs, each hidden behind the last, none catchable by the unit tests, all caught by actually firing the endpoint against production and reading what came back.

**"Don't trust silent successes."** The trap #14 warning earned its place twice this week. `edges:0` isn't an error — it's indistinguishable from "genuinely nothing found." Only timing the call (4.5s) and pulling the logs separated a real answer from a swallowed failure. And later, the Tier-D "map" seed I built to populate the graph 500'd `/graph` for every company with business-intelligence data — but my first post-merge check ran AXTI, whose BI cache happened to be empty, so the seed returned early and looked clean. AAPL, with a populated cache, was the one that hit the bug (a Postgres `date` object where a string was required). The lesson filed under Diagnostic methodology: *verify a data-populated case, not just the empty one.*

**The insight that made it a product.** The prototype of the reasoning engine — before writing a line of the real service — under-reasoned. Asked to name AXTI's architecture transition, it said "unknown," when AXTI sits squarely in the electrical→optical interconnect story for AI datacenters. The method itself held the answer: *inference generates the hypothesis; documents confirm it.* The **map** (transition, chain) is allowed to be reasoned; the **evidence** (tiers, gates, the fit score) is strictly gated. Split those two in the prompt and the same thin evidence produced a rich five-hop chain and a coherent InP-substrate chokepoint hypothesis — while the fit score stayed an honest **2/24**, a "watch item, not a thesis." The engine reasons boldly and scores conservatively. That's the whole trick, and it's faithful to the method by construction: the fit-score total and the two vetoes are computed in *code*, never taken from the model.

**Diminishing returns, honestly reported.** Broadening ingestion past the 10-K taught its own lesson. The 8-K was gold — AXTI's `Item 1.01` filings named a real customer, Nanjing Casela, that the annual report never did. But 10-Q, S-1, and 20-F came back ~0 for mature filers (their front matter is financial statements; the relationship language sits deeper). I shipped the generalization anyway — it's the foundation, and it catches recent IPOs — but said plainly in the PR that 10-K + 8-K are the workhorses. When the ingestion ROI curve bent, the right move was to stop adding sources and build the *reasoning layer*, which is where the product value actually lives.

**Turning it on, and filling it up.** Enabled on `deepseek-reasoner`, AXTI's thesis generated on the first shot — every section populated, a coherent GaAs→InP transition, five invalidation tests, a forward-financial sensitivity anchored to real GAAP margins — fit an honest 1/24. But it was the *only* stock with a result, because everything runs behind a per-symbol gated refresh. So the last piece was a curated warm: 42 hand-picked chokepoint candidates in AXTI's neighborhood (substrates, photonics, semicap, advanced packaging), a script that fires the live endpoints, results landing in Postgres server-side — a cluster of comparable theses instead of one lonely example.

#### Content hook

*"We built a machine that reasons about a company's supply-chain position — is it a real chokepoint, or just theme exposure? — and grades every claim by the evidence behind it, from an SEC filing you can click through down to an inference on 'the map.' The hard part wasn't the reasoning; the model does that well. The hard part was making it reason boldly about what's plausible while scoring conservatively about what's proven — and never, ever issuing a recommendation. Oh, and three bugs stacked behind each other, each invisible until you fixed the one in front of it, none of which a unit test would ever have caught."*

*Last updated: 2026-07-24 (Episode 46 added — the supply-chain bottleneck thesis engine: three stacked bugs, the map-vs-evidence split, and the warm).*
