...
| **Blocks 5+6 — catalysts + themes (home, below the fold)** | Last of Jimmy's six-block spec, merged into one 今天炒什麼-style block (theme + the news reason — a theme with no "why" is thin, a catalyst with no theme is noise). `news_catalyst` and `catalyst_materiality_score` already exist in `sentiment_signal_summaries`; the gap is that **nothing populates them in bulk** (computed on-demand per symbol), plus there is no market-wide read endpoint — every sentiment route is `/{symbol}/...`. | **BLOCKED on the Alpha Vantage tier.** Costs 1 AV news call + 1 LLM call per symbol (LLM skipped under 3 articles). On the free tier (25 req/day) the nightly job cannot exist at any useful size; on premium it is trivial. Design is sized to ~60–80 symbols (today's movers + theme constituents), NOT the R3000 — "what's firing today" is about notable names. **DO NOT pick this up as "the next block"** — the home-IA sequence reads 1→6 and a future session will reach for it in order, but it cannot be built on the current tier. Blocks 1–4 are done; 5+6 wait on the tier decision, which is Jimmy's, not an engineering task. |
...
| **Fundamentals have no bulk refresh — the backfill is a manual chore that must be repeated** | `SymbolCache.pe_ratio` / `dividend_yield` / `beta` / 52w range are written by exactly two paths: `fundamental_service._upsert_symbol` on a **company-page view** (one symbol, only when older than `_CACHE_TTL_HOURS = 24`), and `scripts/backfill_fundamentals.py` **run by hand**. There is no cron in `app/jobs/` and no lifespan warmup for it. So names people visit stay fresh for free and the long tail keeps whatever the last manual run wrote, indefinitely — a stock that turns profitable next quarter stays `pe_ratio = NULL`, invisible to every P/E screen, until someone opens its page. **Measured 2026-08-09** after the R3000 backfill (1011 updated / 299 unprofitable / 10 failed): of 2,552 R3000 names, **793 still have no P/E** and **22 still hold pre-#283 percent-shaped dividend yields**. The 22 are proof rather than inference — `_upsert_symbol` writes `dividend_yield` *unconditionally* (only P/E is guarded), so a surviving percent value means that row's profile write never landed; they were marked done by the pre-#297 checkpoint bug that recorded failures as successes. META, MA and INTC are in that set. **Interim remedy** (prepared, not yet run): a skip-list state file targets exactly the 798 suspects — `--state-file` with every healthy R3000 symbol pre-listed as done, ~45 min at 2 FMP calls/symbol, idempotent and resumable. **Real fix:** a nightly job over the standing universe reusing the script's logic, staggered so it doesn't burn the FMP quota in one burst. Same missing-persistence shape as the template-performance store (§4). | ~2h for the cron; ~45 min per manual run in the meantime | **Repeat the manual backfill every 1–2 months** until the cron exists. Build the cron when a P/E screen returns visibly wrong results, or when the first user asks why a profitable company has no P/E. Verify with `GET /api/screener/by-symbols?symbols=…` and count `pe_ratio: null` + `dividend_yield > 0.5` — those two counts ARE the health metric. |
