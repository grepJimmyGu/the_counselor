...
| **Screen result page redesign** | Jimmy's 2026-08-07 reference: dense sortable table with switchable column groups (相关/概览/表现/技术/估值), match count at top, add-to-watchlist + export. Today it is a card-per-symbol list with verbose reading chips and no numbers. | Not started. Bigger than a block — every home block eventually lands here, and the P/E backfill is what makes the valuation columns possible. |
