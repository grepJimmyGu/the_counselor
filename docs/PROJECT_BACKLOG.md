...
| **Blocks 5+6 — catalysts + themes (home, below the fold)** | Last of Jimmy's six-block spec, merged into one 今天炒什麼-style block (theme + the news reason — a theme with no "why" is thin, a catalyst with no theme is noise). `news_catalyst` and `catalyst_materiality_score` already exist in `sentiment_signal_summaries`; the gap is that **nothing populates them in bulk** (computed on-demand per symbol), plus there is no market-wide read endpoint — every sentiment route is `/{symbol}/...`. | **BLOCKED on the Alpha Vantage tier.** Costs 1 AV news call + 1 LLM call per symbol (LLM skipped under 3 articles). On the free tier (25 req/day) the nightly job cannot exist at any useful size; on premium it is trivial. Design is sized to ~60–80 symbols (today's movers + theme constituents), NOT the R3000 — "what's firing today" is about notable names. |
...
| **Screen result page redesign** — *mostly SHIPPED; two pieces left* | Jimmy's 2026-08-07 reference: dense sortable table with switchable column groups, match count at top, add-to-watchlist + export. Was a card-per-symbol list with verbose reading chips and no numbers. **Shipped** as `/screen` (`components/screen/query-results.tsx`): search box retained at top, condition chips with parallel per-condition counts, default price/change/cap/volume columns, one column per screened condition carrying its VALUE, global ranking (quotes capped at 300, disclosed), 25-row paging, feedback footer (#303); plus the additional-metrics picker — quote / fundamental / technical column groups with range filtering and ranking on any column (#305). | **Remaining: (1) add-to-watchlist + export, (2) "add ticker" — hand-add symbols to a result set, the other half of Jimmy's item 3.** Valuation columns are live but only as good as the fundamentals data — see the bulk-refresh row in §5. |
