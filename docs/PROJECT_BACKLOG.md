...
| **PRD-19 Phase B frontend (Steps 5+6)** — NotificationBanner + MarkAsExecutedButton + NotInvestmentAdviceFooter bricks; NotificationSettingsForm + `/account/notifications` page | Backend is complete and stable. Need a `useSession()`-aware brick for Mark-as-Executed (trap #19), an in-app banner that polls `/api/me/notifications/pending` (route shipped in PR #146), and a settings form calling `GET/PATCH /api/me/email-preferences` with the 3 new PRD-19 flags. PRD shape in [`build_specs/PRD-19_notification_phase_b.md`](../build_specs/PRD-19_notification_phase_b.md) §6 (frontend bricks). | Pickup next session. Worktree pattern: `git worktree add ../the_counselor-prd19-frontend-bricks -b claude/feat/notification-bricks main`. Types-first in `apps/web/src/lib/contracts.ts` per pre-push #2. ~6-8h estimated. |
