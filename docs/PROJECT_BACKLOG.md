...
| Signals v0 Phase B (daily cron + email alerts + signal-unsub) — paused for reshape | Originally shipped as PR #88 on 2026-05-26, unnecessarily reverted via PR #101 during outage-debugging when a Railway *deployment* ID was misread as a git SHA (full post-mortem: `docs/KNOWN_ISSUES.md`, entry 2026-05-26). Jimmy paused the re-apply because Phase B may need a shape change before next attempt — likely targets: (a) replace `asyncio.run()` inside `compute_current_signal` with a cleaner async/sync boundary, (b) consider how Phase B integrates with the still-evolving signal-panel UX from Phase C, (c) re-evaluate whether the 22:00 UTC cron is the right cadence vs. on-demand recompute. Phase A (PR #83) stays on main — signal subscribe / unsubscribe endpoints, the 3 DB tables, and the `users.has_seen_signal_intro` column are unaffected. | Resume when Jimmy is ready to reshape Phase B. The original code is preserved on the `claude/feat/signals-v0-phase-b-cron-email` GitHub remote branch (already merged + revert-merged on main) — `git show 9eeb3a9` for the original implementation diff. The fix for the underlying conn-leak bug that *might have been blamed on* Phase B (Stripe-in-DB-tx in `dunning_expiry_job`) shipped separately in PR #104, so re-applying Phase B as-is should no longer carry that risk. |
