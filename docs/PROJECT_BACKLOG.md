...
- **Extensible prewarm-universe registry (+ Nasdaq-100 as the first add)** [Jimmy, 2026-06-16; live demo confirmed the S&P-only snapshot]: today the standing universe is **hardcoded to `SP500_TICKERS` in ~4 places** — the warm job (`signal_snapshot_job._warm_sp500`), the resolver (`universe_resolver.resolve_universe` + `is_standing_universe`), the scan-request validator (`screener_scan._FIXED_UNIVERSE_IDS`), and the UI (`universe-selector.tsx` `TIERS`). **Goal: collapse to ONE source of truth** — a `STANDING_UNIVERSES` registry (`universe_id -> ticker frozenset`, e.g. `app/data/standing_universes.py`) that the warm job (warm the **union** of all registered sets — the snapshot is keyed by symbol, universe-agnostic), resolver, validator, and a new `GET /api/screen/universes` endpoint all read; the UI fetches that list instead of hardcoding tiles. Then **adding a universe = (1) one registry entry + (2) make sure its `price_bars` stay warm** — not a multi-file edit.
  ...
  - **Storage headroom — verified 2026-06-18 (NO disk bump needed):** Postgres volume **5,000 MB, 1,751 MB used (35%, ~3.25 GB free)**; `price_bars` is 1,128 MB / 1,270 symbols / **~206 bytes/row**. A 3-yr backfill is ~750 bars/symbol → **R1000 ≈ +75–150 MB** (≤475 net-new vs the ~525 SPX already warm), **R3000 ≈ +380 MB**. Both land under ~45% of the volume — trap #10 (disk-fill) is **not** a constraint at current scale. (Inspect: `railway volume list` + size queries via the Postgres `DATABASE_PUBLIC_URL`.)
  - **Backfill is plug-and-play (parametrized 2026-06-18):** `scripts/backfill_sp500_universe.py` now takes `--tickers-file <list>` (any universe; idempotent — skips already-fresh symbols) + `--lookback-years` (default 3); the SP500 default is unchanged. So the **data-load half is ready**: drop the ticker list → run against prod (`DATABASE_PUBLIC_URL` + `ALPHA_VANTAGE_API_KEY`; AV ~75/min → ~6–10 min for ~475 R1000 net-new). Remaining for an actual R1000 add = a `russell1000_tickers.py` + the `STANDING_UNIVERSES` registry (resolver/validator/UI) above + daily keep-warm wiring + a liquidity-floor on presets. **Sequence the warmup-deadlock fix (below) first/alongside** — a bigger daily warm widens that contention window.
