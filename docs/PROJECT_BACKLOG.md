...
| **Blocks 5+6 — catalysts + themes (home, below the fold)** | Last of Jimmy's six-block spec, merged into one 今天炒什麼-style block (theme + the news reason — a theme with no "why" is thin, a catalyst with no theme is noise). `news_catalyst` and `catalyst_materiality_score` already exist in `sentiment_signal_summaries`; the gap is that **nothing populates them in bulk** (computed on-demand per symbol), plus there is no market-wide read endpoint — every sentiment route is `/{symbol}/...`. | **BLOCKED on the Alpha Vantage tier.** Costs 1 AV news call + 1 LLM call per symbol (LLM skipped under 3 articles). On the free tier (25 req/day) the nightly job cannot exist at any useful size; on premium it is trivial. Design is sized to ~60–80 symbols (today's movers + theme constituents), NOT the R3000 — "what's firing today" is about notable names. **NOT the same thing as the `Catalysts` block on Home** (shipped #319, 2026-08-13) — that one surfaces the five `kind: "sentiment"` starting points over the news ticker and needs no per-symbol AV/LLM work. This row is the 今天炒什麼 theme+catalyst read, which still needs bulk `news_catalyst` population. **DO NOT pick this up as "the next block"** — the home-IA sequence reads 1→6 and a future session will reach for it in order, but it cannot be built on the current tier. Blocks 1–4 are done; 5+6 wait on the tier decision, which is Jimmy's, not an engineering task. |
| **Example-query strings (home block 4)** | `home-example-queries.tsx` ships with DRAFT copy chosen because it parses, not because it is how a trader talks. 問財's equivalent works because the questions sound like someone speaking. | Awaiting Jimmy's own strings — 3–4 per tab. One-line edits; `tests/test_home_example_queries.py` is the guard rail (every string must parse or it falls through to the LLM, which interrogates instead of screening). |
| **Screen result page redesign** — *Jimmy's 4-point spec SHIPPED; one piece left* | Jimmy's 2026-08-07 reference: dense sortable table with switchable column groups, match count at top, add-to-watchlist + export. Was a card-per-symbol list with verbose reading chips and no numbers. **Shipped** as `/screen` (`components/screen/query-results.tsx`): search box retained at top, condition chips with parallel per-condition counts, default price/change/cap/volume columns, one column per screened condition carrying its VALUE, global ranking (quotes capped at 300, disclosed), 25-row paging, feedback footer (#303); fundamentals-by-symbol backend (#304); additional-metrics picker — quote / fundamental / technical column groups with range filtering and ranking on any column (#305); hand-add a ticker, URL-backed via `&add=`, badged, never counted as a match (#307). | **Remaining: add-to-watchlist + export.** Nothing blocks it. Valuation columns are live but only as good as the fundamentals data — see the bulk-refresh row in §5. |
| **One results surface for both query engines** — *scoped 2026-09-10, not built* | Home's search box and its "Traders ask" chips answer a question two different ways. A TECHNICAL query ("golden cross") lands on `screen_results` — basket, per-row "why this matched" chips, `as_of` byline, rank-by-return, promote-to-strategy. A FUNDAMENTAL one ("p/e under 15") lands on `/stocks/screener`, a different page with different chrome. Same box, same affordance, two products. **The scan half already supports unifying them**, verified against production: `POST /api/screen/scan {universe_id:"symbols", rules:[], symbols:[…]}` → `200 {matched:[…], readings:{}, as_of_date}`. So a fundamental basket renders on `screen_results` today. | **What blocks it is the RANK half, and it is a real asymmetry, not a wiring gap.** `rank` requires a `strategy`, and a fundamental query returns `strategy_json: null` — there are no rules to backtest per symbol, so "rank by the return of applying these rules" has no meaning for "P/E under 15". Scope is therefore **a no-rules mode on `ScreenResults`**: render the basket; render `fundamental_filters` (e.g. `["P/E under 15"]`) as a screen-level "why" in place of per-row `readings`, which come back `{}`; hide or repurpose the ranked columns and the "sign in to rank" slot. Everything else on that surface already works. Do NOT close this by pointing the fundamental branch at `screen_results` without that mode — the page would render a basket with an empty reason column and dead rank columns, which is worse than the two surfaces it replaces. Prerequisite fix already shipped in #366 (the branch pointed at `/stocks`, which has been Market Pulse since 2026-05-15 and reads no params — 543 resolved names were discarded in the browser). |
| **Template performance store** | Block 3 (quant strategies) deliberately ships with NO performance numbers: `perfContext` on templates reads like backtested returns but is hand-written prose, and no per-template store exists (`strategy_live_performance` is slug-keyed and lacks the columns; `BacktestRecord` has the metrics but never persists `template_id`). Cards show `evidenceTier` instead — a sourced claim about the idea, not an invented number. | Needs a store + a nightly backtest job. **Never surface `perfContext` as backtested.** |
...
| ~~Signal-triggered ENTRY~~ | ~~Daily cron notifies "your strategy says enter X" → user buys + confirms a fill → declared position.~~ | **DONE 2026-08-19.** Smaller than written: detection already existed — `signal_cron` computes each strategy's signal daily and `flip_to_long` IS the entry signal, emailed and bannered like any other change. The gap was that its action link pointed at `?action=executed`, the same target an EXIT sends, so the user met "nothing has been sold… confirm what you sold". Entry links now carry `?action=entered` and land on `<EnteredFromEmail>`, which routes to the declare form. Structural, not cosmetic: an entry that never becomes a declared `PositionState` is invisible to the exit monitor, so the ladder the user configured never ran on the trade it was built for. |
...
**PRD-23c status (2026-06-17) — Discover → Track:** PR1 (#220, save→track→notify backend core) + PR2 (#221, reads + working "Save + track" CTA) **shipped + merged**. A Strategist+ user can save a standing screen and the 23:30-UTC cron alerts them on each NEW basket entrant (transition-only). **Remaining:**
- **PR2c — saved-screen view + rough-edge fix (NOT optional; PRD-23c §3.3 DoD).** PR2 left a gap: a saved screen is a `SavedStrategy` (`kind="screen"`), and `GET /api/strategies` (`list_saved_strategies`, `app/api/routes/saved_strategies.py:54`) returns ALL of a user's SavedStrategies with **no filter** → a saved screen **leaks into "My Strategies"** and would render broken on the strategy-detail page (which expects a backtest). Fix: **(a)** filter `kind=="screen"` out of the strategies list (or route screens to their own view); **(b)** ship the standalone **`/screens/[id]` dashboard** (current basket + entrant/exit history via `GET /api/screen/saved/{id}`) + a **"My Screens"** list. The backend reads already exist (#221: `GET /api/screen/saved` + `/saved/{id}`).
- **PR3 — intraday snapshot (genuinely optional; the spec's "the option").** Warm a `resolution='intraday'` `signal_snapshot` on the PRD-16c FMP cadence so the scan runs mid-session; tier-gate intraday screens. Daily screening already works — additive value, not a gap.
- **Editorial follow-up:** `_db_sector_membership` is duplicated (route `screen.py` + `saved_screen_service.py`) — unify into one shared screener helper (the same sector-label-normalization TODO applies to both).
...
- ~~**Sector-label normalization**~~ ✅ **SHIPPED #250 (2026-06-25),** then ✅ **RE-FIXED AT THE SOURCE (2026-08-05).** Canonical GICS end-to-end: `app/data/russell3000_sectors.py` + `POST /api/admin/backfill/sectors` normalize `SymbolCache.sector`; the picker offers GICS labels; `_db_sector_membership` intersects the standing union. Fixed a verbatim-match bug that had silently broken 6/11 sectors (picker sent FMP labels, DB stored GICS).
  - **Why it came back:** #250 normalized the *data* but not the *write path*. `fundamental_service._upsert_symbol` re-wrote `sector` from the FMP profile on **every company-page view**, so rows drifted back to FMP labels one page view at a time — 64 rows in the six weeks after #250, plus 518 rows carrying the literal string `'nan'` from `seed_symbols.py` (pandas `float('nan')` is truthy, so its `or ""` guard never fired). The lesson is the general one: normalizing stored data without normalizing the writer buys weeks, not a fix.
  - **Now:** `app/data/sectors.py::normalize_sector()` is the single choke point all three writers go through, `scripts/normalize_symbol_sectors.py` repairs existing rows, and `tests/test_sector_normalization.py` pins the invariant. `value_chain_classifier` was re-keyed to GICS in the same PR — it had been keyed on FMP labels while reading GICS rows, so value-chain role and cyclicality were silently `None` for most companies.
- ~~**Extensible prewarm-universe registry (+ Nasdaq-100 as the first add)**~~ ✅ **REGISTRY SHIPPED #249 (2026-06-25).** `app/data/standing_universes.py::STANDING_UNIVERSES` is the single source of truth — the resolver, scan/save validators, the daily warm (warms the **union**), and the frontend tiles all read it. **Russell 3000 was the first add** (bars backfilled via the #248 job, warmed live). NOTE: the proposed `GET /api/screen/universes` endpoint was NOT built — the frontend still hardcodes the 2 standing tiles (fine until there are many).
  - **Nasdaq-100 is now trivial** — one `STANDING_UNIVERSES` entry + a one-time `price_bars` backfill for the ~15–25 NDX names not already warm (`POST /api/admin/backfill/universe`), then `POST /api/admin/snapshot/warm`. Market Pulse "NDX Top Movers" reuse still available (`_build_top_assets(NASDAQ100_TICKERS)`, additive, SPX untouched — expand-only invariant).
  - **R3000 deferred fast-follows (2026-06-25, agreed):** (1) **liquidity floor** on R3000 presets — microcaps surface junk in `best_momentum`-type screens; shipped raw deliberately, tune the floor against real results. (2) reconcile the **6 AV-drift class shares** (AKE, BF.A, BF.B, GEFB, HEIA, LENB — AV wants the hyphen form, e.g. `BF-A`) if their bars are wanted. **The FMP half of this is fixed** — PR #295 moved the dot→hyphen translation into `FMPClient._get` after an audit found 7 of 10 symbol-taking methods missing it; only a single-letter suffix is translated, so CN tickers (`000001.SZ`) are untouched. The *Alpha Vantage* side is still open. (3) `pct_below_high` primitive (listed under Composer flexibility above).
...
| Rate-limit the public `POST /api/search/parse` | Unauthenticated and LLM-backed on the fallback path — real cost/abuse exposure. Noted repeatedly since 2026-07-30. | ~1h | Before any public launch push, or on the first anomalous spend |
| Chinese queries silently fall through to the LLM | The 中文 toggle switches UI copy only; `screen_rule_parser`/`screen_filter_parser` are English-only regex, so 均线多头排列 extracts nothing. **Jimmy explicitly declined to invest in Chinese parsing 2026-08-07.** `tests/test_query_vocabulary.py` documents the gap. | Own project | Only if CN becomes a priority — do NOT start this as a "fix" |
...
| **Audit catalog primitives that are present in the snapshot but never populate** — *guard SHIPPED; run the sweep post-deploy* | **Guard landed** (`signal_snapshot_service.py`): (1) `DEGENERATE_SNAPSHOT_PRIMITIVE_IDS` denylist excludes `rank_composite_score` from the warmed + scannable vocab — a rule over it now reports `unsupported` (explicit) instead of silently matching 0; (2) `compute_snapshot_coverage()` computes per-primitive coverage; (3) `warm_universe` runs it after every warm and **logs a WARNING** naming any primitive that's all-null/all-zero across the universe. **Remaining:** after this deploys + the snapshot warms, read the Railway log line `signal_snapshot coverage: N primitive(s) degenerate …` and add any newly-surfaced ids to `DEGENERATE_SNAPSHOT_PRIMITIVE_IDS` (a one-line change each). Other dead primitives beyond `rank_composite_score` are still unknown until that sweep runs against live data. | ~15 min (read the warm log + encode findings) | **Before** adding more composer-backed screener presets (PRD-24a §6) — but the denylist + warning now make the trap loud instead of silent. |
| **Fundamentals have no bulk refresh — every gap needs a manual run, and drift resumes the same day** | `SymbolCache.pe_ratio` / `dividend_yield` / `market_cap` / `beta` / 52w range are written by exactly two paths: `fundamental_service._upsert_symbol` on a **company-page view** (one symbol, only when older than `_CACHE_TTL_HOURS = 24`), and `scripts/backfill_fundamentals.py` **run by hand**. There is no cron in `app/jobs/` and no lifespan warmup. So coverage is an accident of which script ran last and whose pages people opened. **Measured 2026-08-13** on a seeded 400-symbol sample of the R3000, before and after that day's run (2542 updated / 728 genuinely unprofitable / 26 failed, **3h27m wall clock**): `sector` 100% → 100%, `pe_ratio` 70.5% → 70.8%, `dividend_yield` 63.2% → 63.0%, **`market_cap` 1.8% → 99.2%**. Market cap was the real emergency and nobody had measured it: `backfill_fundamentals.py` was already fetching the profile that contains it and discarding the value (fixed #318), while four presets and both market-cap screener filters read it — `min_market_cap=2e9` returned 99 names before and 1,677 after. The pre-#283 percent-shaped dividend rows are gone (0 remaining), and #317 added a write guard (`sane_dividend_yield`) plus a read guard on both screener filter sites so such a row cannot be reported as a yield even if one reappears. **Real fix — the whole point of this row:** the manual loop is structural, not a chore. A full sweep is ~5,100 FMP calls at ~5s/symbol = **3.5 hours**, which rules out a nightly full refresh and argues for **lazy refresh driven by traffic**: a screen returns immediately from stored data and queues the N stalest symbols in that universe for background refresh, with a cron only as a floor so untouched corners still converge. Self-healing, proportional to use, nobody runs anything. Same missing-persistence shape as the template-performance store (§4). | ~1 day for lazy refresh + coverage reporting; ~3.5h per manual run in the meantime | **Design agreed in principle 2026-08-13, not yet specced.** Sizing depends on the FMP plan's rate ceiling — ask Jimmy before choosing a tick rate. Health metric: sample 400 R3000 names via `GET /api/screener/by-symbols` and count non-null per column; surface it on `/health` so a gap is loud rather than something a person trips over. |
| **`price_bars` holds no ETFs or ADRs — the Timing Engine is blind to whatever a user trades outside the equity universe** | The warmed universe is S&P 500 + Russell 3000 *equities*. Measured 2026-08-28 on the live account by direct query, during the PRD-43b P0 live run (#356): **zero rows** for `UVXY`, `SQQQ`, `SVXY`, `VIXY`, `VIXM`, `GOLY`, `ICVT`, `BHP`, `UMC`, `ERO` — and none for `^VIX` or `VIX` under either ticker. That is **10 of 33 traded symbols**, and it cost **24 of 57 episodes**: `UVXY` alone carried 15. So the WHEN section measured 58% of that account's record and silently omitted its most-traded instrument. The engine degrades correctly (`no_price_history`, named per symbol) rather than fabricating, so this is a **coverage** problem, not a correctness one — but for a vol-ETF trader the lens is close to useless, and 43c's allocation weights will inherit the same hole. **Two parts:** (a) backfill common ETFs/ADRs — the leveraged and inverse vol complex, sector and index ETFs, the ADRs that show up in retail books; (b) `^VIX` specifically, which PRD-43b §3.5 listed as *unverified* and is now **verified absent**, so the market-context group can never populate its volatility feature. ⚠ Trap #10 applies — check Railway disk headroom before ingesting; the R3000 backfill filled the volume once already. | ~2–4h for a scoped ETF list + a backfill run; unknown for a general "whatever the user traded" ingest | **Deferred by Jimmy 2026-08-28** — do not start speculatively. Trigger: the WHEN or HOW MUCH section ships to a real user whose coverage is under ~80%, or the first support report of "my biggest position isn't in here." Until then the honest fix is copy: the surface must state the measured share, never imply the whole record. |
| **Startup-warmup Postgres deadlock occasionally fails a deploy** | The concurrent lifespan warmups can deadlock on Postgres. On 2026-06-18 deploy `c570b80e` failed: `psycopg.errors.DeadlockDetected` in `_warmup_market_pulse_loop` (`main.py:543`) poisoned the shared transaction → `InFailedSqlTransaction` cascade → healthcheck timeout → container stopped. **It self-heals** — Railway's retry (`00660056`, same code) started clean — so prod stays up on the prior container (trap #11 pattern), but a deploy is marked failed and the new code lands one retry late. Intermittent (a lock-ordering race between the market-pulse warmup and the snapshot/GSPC warmups, all touching `price_bars`). **NOT caused by any one PR** (the #240 snapshot-audit code is absent from the trace; if anything it reduces warm work). Fix options: serialize/stagger the lifespan warmups so they don't contend, OR add a bounded deadlock-retry on the market-pulse warmup tick. Sensitive `main.py` lifespan area — traps #21/#22 apply (don't share asyncio primitives / don't block the loop). | ~1–2h (stagger warmups + verify under a cold deploy) | When a deploy fails the healthcheck and the logs show `DeadlockDetected` in a warmup. Until then it's a self-healing annoyance, not an outage. |
