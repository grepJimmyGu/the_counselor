...
| **Audit catalog primitives that are present in the snapshot but never populate** | `rank_composite_score` is returned by `snapshot_primitive_ids()` yet evaluates to 0/null at *every* threshold across the whole SP500 universe — so any Market Screener preset/scan rule using it silently matches **0** with no error (surfaced while fixing PRD-24a's `best_momentum` preset, PR #234, which shipped matching 0/525 until this was caught by a live curl). Add a guard: a test or a `signal_snapshot` warm-time assertion that flags any primitive whose snapshot column is **all-null/all-zero across the warmed universe**, so "present-but-degenerate" primitives can't ship inside a preset. Needs a one-time sweep of all ~93 snapshot columns — other dead primitives beyond `rank_composite_score` are unknown. | ~1–2h (sweep + a guard test) | **Before** adding more composer-backed screener presets (PRD-24a §6 / Phase 2) — every new preset risks the same silent-0 trap. |
