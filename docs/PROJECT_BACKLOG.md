...
| Packet | Scope | Status |
|---|---|---|
| **Custom Mode (PRD-16a / 16b / 16c)** — Signal Library + Composer + Active Execution | 3-PRD sequential packet, 5–7 weeks total. Takes Custom Mode from "fork a template and tweak risk" to "compose any combination of ~55 signal primitives, backtest on daily or intraday bars, and optionally run live with multi-tier exits + dashboard." HANDOFF doc: [`agent-system/plans/HANDOFF-livermore-custom-mode.md`](../agent-system/plans/HANDOFF-livermore-custom-mode.md). PRD-16c depends on PRD-19 (done as of 2026-06-09) plus PRD-16a + PRD-16b on `main`. | Queued — Mr Gu signaled "build a new batch" first; pick this up after the next batch lands. Reading order + brick inventory + pitfalls all already documented in the HANDOFF. |
