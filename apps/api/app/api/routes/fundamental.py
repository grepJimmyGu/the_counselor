...
async def get_profile(symbol: str, db: Session = Depends(get_db)) -> CompanyProfile:
...
async def get_key_metrics(symbol: str, db: Session = Depends(get_db)) -> KeyMetrics:
...
async def get_overview(symbol: str, db: Session = Depends(get_db)) -> FundamentalSummary:
