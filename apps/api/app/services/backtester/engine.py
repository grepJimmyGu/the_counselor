...
class BacktestEngine:
    ...
    async def compute_current_position(
        self, db: Session, strategy: StrategyJSON
    ) -> dict:
        """Run the front half of `.run()` (load prices → generate weights) and
        return the final-row position only.

        Used by the Stage 8 v0 signals cron to derive "what does this strategy
        say to do today?" without paying for the full P&L tail of `.run()`.

        Returns a dict with three keys:
          - "weights": dict[str, float] — non-zero (>1e-6) ticker → weight on
            the final date; empty dict means the strategy is in cash
          - "prices": dict[str, float] — close prices on the final date for
            every ticker present in the universe (rounded to 4 decimals)
          - "as_of": date — the actual market-data date that drove the
            computation (may lag the requested `strategy.end_date` by a few
            days when re-running over a weekend / holiday window)

        Limitation (v0): fundamental strategy types (`_FUNDAMENTAL_STRATEGY_TYPES`)
        require the multi-provider precompute block in `.run()`. The cron's
        per-strategy try/except handles the NotImplementedError raised here
        and continues with the rest of the population. Lift this limit in a
        follow-up phase if traction warrants.
        """
        if strategy.strategy_type in _FUNDAMENTAL_STRATEGY_TYPES:
            raise NotImplementedError(
                f"compute_current_position does not support "
                f"strategy_type={strategy.strategy_type!r} in v0 "
                "(fundamental providers deferred)"
            )

        # ─ Validate (cheap; surfaces schema errors before the load-prices fan-out) ─
        list(validate_strategy(strategy))

        # ─ Pipeline mirrors `.run()` lines 808-820 ───────────────────────────
        # Duplicated deliberately — extracting a shared helper would touch the
        # hot `.run()` path and risk regressing existing backtest behavior.
        # Converge later if a third caller materializes.
        ...
        if close_matrix.empty:
            raise ValueError("No overlap between universe and benchmark price history.")

        weights = self._generate_weights(strategy, close_matrix, aligned_frames, {})

        last_idx = weights.index[-1]
        last_row = weights.iloc[-1]
        non_zero = {sym: float(w) for sym, w in last_row.items() if abs(w) > 1e-6}

        last_prices_row = close_matrix.loc[last_idx]
        prices = {
            sym: round(float(p), 4)
            for sym, p in last_prices_row.items()
            if pd.notna(p)
        }
